...
def test_from_label_extraction(raw, expected):
    assert extract_from_label(raw) == expected
...
def test_detail_removal(raw, expected):
    assert remove_details(raw) == expected
...
def test_recieved_by_label_extraction(raw, expected):
    assert extract_received_by_label(raw) == expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("by 10.31.214.5 with HTTP; Tue, 12 Jan 2016 10:20:05 -0800 (PST)", "HTTP"),
        (
...
def test_protocol_extraction(raw, expected):
    assert extract_protocol(raw) == expected
